...
Este é um livro em desenvolvimento. Toda contribuição, seja correção de erros ortográficos ou sugestões de novos conteúdos, será bem-vinda. Você pode contribuir criando *issues* [neste repositório](https://github.com/curso-r/livro-shiny). 
...
Em específico, este livro assume que você já tem conhecimento do operador *pipe* (`%>%`) e dos principais pacotes do tidyverse (`readr`, `dplyr`, `tidyr`,`ggplot2`, `stringr` e `lubridate`). Se esses temas não são familiares para você, indicamos a leitura do livro [Ciência de Dados em R](https://livro.curso-r.com/) antes de prosseguir.
...
- como inserir *htmlwidgets* ao seu app.

Se você já programa em Shiny e gostaria de se aprofundar no tema, tanto para aprender técnicas para customizar o visual do seu aplicativo, quanto para aprender mais sobre reatividade e como o Shiny funciona por trás das cortinas, você pode iniciar no Capítulo \@ref(modulos). A partir desse capítulo, vamos

- utilizar módulos para organizar a construção de aplicativos grandes e complexos;

- construir um app utilizando o framework do pacote `golem`;
