...
Todo esse processo que envolve monitorar mudanças nos inputs para atualizar os outputs a partir de uma receita composta de códigos que devem ser executados na hora certa é o que chamamos de **reatividade**. A melhor parte é que o Shiny cuida sozinho de grande parte desse processo. Tudo o que precisamos fazer é especificar a receita.
...
Os valores reativos são **a origem do diagrama de reatividade**. Eles guardam as informações que vêm da UI e disparam um *sinal de alerta* sempre que essas informações mudam. Esse sinal é enviado a qualquer output do app que dependa desses valores e indica que essas saídas devem ser recalculadas.
...
## O disparo de reatividade

Um aplicativo Shiny é composto por um ou mais outputs que podem ser controlados a partir de um ou mais inputs. Sempre que alteramos o valor de um input na tela, uma mensagem é enviada para o servidor onde o aplicativo está hospedado e, dentro da sessão de R, é recebida pela função `server()`. Essa mensagem contém o novo valor desse input, que é transmitido **apenas para os outputs que dependem dele**. Esses outputs são recalculados e os novos resultados são devolvidos para a UI. Chamamos esse processo de **fluxo de reatividade**.

O fluxo de reatividade de um app é definido por seus inputs, outputs e pelo código contido dentro da função `server()`. Vamos analisar um exemplo simples. O app seguir possui dois inputs (caixas de seleção) e dois outputs (gráficos).
...
O código desse app se enconta a seguir:
...
Quando alteramos o valor da Variável A, o seguinte processo acontece:
...
## Gráfico de reatividade: `reaclog`
