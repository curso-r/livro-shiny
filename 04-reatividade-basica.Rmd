...
Um aplicativo Shiny é composto por um ou mais outputs que podem ser controlados a partir de um ou mais inputs. Sempre que alteramos o valor de um input na tela, uma mensagem é enviada para o servidor onde o aplicativo está hospedado e, dentro da sessão de R, é recebida pela função `server()`. Essa mensagem contém o novo valor desse input, que transmitido **apenas para os outputs que dependem dele**. Esses outputs são recalculados e os novos resultados são devolvidos para a UI. Chamamos esse processo de **fluxo de reatividade**.

O fluxo de reatividade de um app é definido por seus inputs, outputs e pelo código contido dentro da função `server()`.
