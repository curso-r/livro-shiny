...
Embora dê para fazer muita coisa no Shiny na tentativa e erro, entender a reatividade nos desvia de vários erros explícitos  --- como o erro acima --- e, principalmente, dos erros invisíveis --- quando o app funciona, mas de maneira ineficiente ou errada. Economizamos muito tempo (e sanidade) por não incorrer nesses erros durante o desenvolvimento de um app e, por isso, garanto que vale a pena estudar reatividade desde o início da sua jornada pelo Shiny.
...
A reatividade reside em um outro paradigma de programação, conhecido como **programação declarativa**. Nele, não construímos códigos que serão rodados interativamente ou sequencialmente, mas sim uma *receita* que especifica o que o computador deve fazer em um determinado conjunto de situações. Essas receitas geralmente são usadas dentro de um contexto específico, um ambiente que sabe qual código deve ser executado e quando.

No contexto do Shiny, essa receita é um conjunto de dependências que será utilizado para decidir quais outputs devem ser recalculados quando um input muda. Não somos nós que rodamos ou decidimos quando rodar o código que gera um output, nós apenas definimos qual código será esse. O ambiente criado pelo Shiny (quando um app está em funcionamento) é quem decide a hora certa de executar esse código (se é que ele será executado).

Todo esse processo, que envolve monitorar mudanças nos inputs para atualizar os outputs a partir de uma receita composta de códigos executados apenas quando necessário, é o que chamamos de **reatividade**. A melhor parte é que o Shiny cuida sozinho de grande parte desse processo. Tudo o que precisamos fazer é especificar a receita.

Essa receita nada mais é do que a função `server()`, isto é, são os códigos que escrevemos na função `server()` que determinam as dependências entre inputs e outputs. Chamaremos aqui o conjunto de dependências de um app de **diagrama de reatividade**. 
...
No restante deste capítulo, aprenderemos alguns conceitos e funções que nos permitirão ter um controle maior sobre a reatividade dos nosso aplicativos. Não esgotaremos o assunto reatividade aqui, ele será retomado no Capítulo XX, mas já sairemos capazes de construir divertos tipos de apps diferentes. 
...
- o valor reativo `input$variavel_A` emite um sinal de alerta a todos os observers que dependem dele, no caso, à `renderPlot()` responsável pelo `histograma_A`;
...
- o novo histograma gerado e enviado para a UI, encerrando o processo reativo.
...
Os valores da lista `input` não são o único tipo de valores reativos, assim como as funções `render*()` não são o único tipo de observers. Falaremos mais de outros valores reativos e observers mais adiante no livro.

## Expressões reativas

Muitas vezes, um aplicativo shiny precisa de passos intermediários, entre o input de origem e o output final. Isto é, precisamos de uma estrutura que receba um valor reativo, faça alguma conta e devolva um resultado, um valor também reativo que será utilizado posteriormente em um ou mais *observers*. 

Essas estruturas são as **expressões reativas**, a peça faltante do problema apresentado na introdução desse capítulo. 


Para criá-las, utilizamos as funções `reactive({})` e `eventReactive({})`.
...
Para acessar o valor dessa expressão reativa, devemos usar parênteses após o nome, como se fosse uma função sem argumentos. Podemos utilizar esse valor dentro de qualquer outra expressão reativa ou observer, quantas vezes for preciso.
...
A função `eventReactive()` também cria expressões reativas, mas que observam as mudanças de apenas um valor reativo. Esse valor reativo é especificado como primeiro argumento da `eventReactive()`, enquanto o código da expressão é passado no segundo argumento.
...
Vimos na introdução do capítulo que a nossa segunda tentativa, colocar a criação da amostra diretamente na função `server`, retorna um erro. Isso acontece porque **valores reativos e expressões reativas só podem ser utilizadas dentro de um contexto reativo**. 
...
Para evitar esses problemas, que discutiremos em detalhes nos Capítulos XXX, podemos imaginar ou desenhar o diagrama de reatividade para investigar onde está a falha. Essa é uma tarefa simples em apps com poucos inputs e outputs, mas extremamente difícil ou inviável em apps complexos.

Nesses casos (ou mesmo nos casos simples), podemos utilizar o pacote `reactlog`. Com ele, podemos visualizar facilmente o diagrama de dependências reativas de qualquer Shiny app e olhar o que acontece por trás das curtinas da reatividade quando executamos o aplicativo.
